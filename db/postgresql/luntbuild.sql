...
CREATE TABLE LB_SCHEDULE(ID BIGINT NOT NULL PRIMARY KEY,FK_PROJECT_ID BIGINT NOT NULL,SCHED_DISABLED VARCHAR(255) NOT NULL,NAME VARCHAR(255) NOT NULL,DESCRIPTION VARCHAR(255),NEXT_VERSION VARCHAR(4096),QUARTZ_TRIGGER BYTEA,DEPENDENT_SCHEDULE_IDS BYTEA,BUILD_NECESSARY_CONDITION VARCHAR(255),ASSOCIATED_BUILDER_NAMES BYTEA,ASSOCIATED_POSTBUILDER_NAMES BYTEA,BUILD_TYPE INTEGER,LABEL_STRATEGY INTEGER,NOTIFY_STRATEGY INTEGER,POSTBUILD_STRATEGY INTEGER,TRIGGER_DEPENDENCY_STRATEGY INTEGER,STATUS INTEGER,STATUS_DATE TIMESTAMP,BUILD_CLEANUP_STRATEGY INTEGER,BUILD_CLEANUP_STRATEGY_DATA VARCHAR(255),WORKING_PATH VARCHAR(255),CONSTRAINT SYS_CT_9 UNIQUE(FK_PROJECT_ID,NAME));
