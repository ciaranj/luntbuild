...
CREATE TABLE LB_USER(ID BIGINT NOT NULL PRIMARY KEY,NAME VARCHAR(255) NOT NULL,PASSWORD VARCHAR(255),CAN_CREATE_PROJECT SMALLINT,CONTACTS BLOB,FULLNAME VARCHAR(255),CONSTRAINT SYS_CT_3 UNIQUE(NAME));
CREATE TABLE LB_PROJECT(ID BIGINT NOT NULL PRIMARY KEY,NAME VARCHAR(255) NOT NULL,DESCRIPTION VARCHAR(255),NOTIFIERS BLOB,VCS_LIST BLOB,BUILDER_LIST BLOB,VARIABLES VARCHAR(2048),LOG_LEVEL INTEGER,CONSTRAINT SYS_CT_5 UNIQUE(NAME));
CREATE TABLE LB_VCS_LOGIN(ID BIGINT NOT NULL PRIMARY KEY,FK_PROJECT_ID BIGINT NOT NULL,LOGIN VARCHAR(255) NOT NULL,FK_USER_ID BIGINT NOT NULL,CONSTRAINT SYS_CT_7 UNIQUE(FK_PROJECT_ID,LOGIN));
CREATE TABLE LB_SCHEDULE(ID BIGINT NOT NULL PRIMARY KEY,FK_PROJECT_ID BIGINT NOT NULL,SCHED_DISABLED SMALLINT, NAME VARCHAR(255) NOT NULL, DESCRIPTION VARCHAR(255),NEXT_VERSION VARCHAR(4096),NEXT_VERSION_VALUE VARCHAR(4096),VARIABLES VARCHAR(2048),QUARTZ_TRIGGER BLOB,DEPENDENT_SCHEDULE_IDS BLOB,BUILD_NECESSARY_CONDITION VARCHAR(255),ASSOCIATED_BUILDER_NAMES BLOB,ASSOCIATED_POSTBUILDER_NAMES BLOB,BUILD_TYPE INTEGER,LABEL_STRATEGY INTEGER,NOTIFY_STRATEGY INTEGER,POSTBUILD_STRATEGY INTEGER,TRIGGER_DEPENDENCY_STRATEGY INTEGER,STATUS INTEGER,STATUS_DATE TIMESTAMP,BUILD_CLEANUP_STRATEGY INTEGER,BUILD_CLEANUP_STRATEGY_DATA VARCHAR(255),WORKING_PATH VARCHAR(255),CONSTRAINT SYS_CT_9 UNIQUE(FK_PROJECT_ID,NAME));
