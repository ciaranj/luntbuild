   ...
   private
    ...
    def call(methodName, args)
      ...
      begin
        ...
        request = Net::HTTP::Post.new(@requestURI, header)
        request.basic_auth(@username,@password) unless @username.nil?
        http.request(request, data) {|response|
...
##########################################################
  class Deserializer 
    ...
    def deserialize(s) 
      ...
      tag = @is.readNextChar
      raise HProxyException.new("Invalid tag, expected 'r', received '" + tag + "'") if (tag != 'r') 
      ...
      tag = @is.readNextChar
      raise HProxyException.new("Invalid tag, expected 'z', received '" + tag + "'") if (tag != 'z') 
   ...
   private 
    ...
    def parseObject()   
      ...
      raise HProxyException.new("Invalid tag: '" + tag + "'")
    end
