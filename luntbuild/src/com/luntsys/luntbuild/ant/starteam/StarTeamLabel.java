...
public class StarTeamLabel extends StarTeamTask {
	...
	public void setLastBuild(OLEDate lastbuild) throws BuildException {
		this.lastBuild = lastbuild;
	}

	private Label findLabel(View v, String label) throws BuildException {
		Label[] allLabels = v.fetchAllLabels();
		for (int i = 0; i < allLabels.length; i++) {
			Label stLabel = allLabels[i];
			if (stLabel != null && stLabel.getName().equals(label)) {
	...
	public void execute() throws BuildException {
		...
		try {
			...
			if (findLabel(snapshot, this.labelName) != null) { // label already exist
				log("Label \"" + this.labelName + "\" already exist, creation skipped...", Project.MSG_WARN);
				return;
			}
