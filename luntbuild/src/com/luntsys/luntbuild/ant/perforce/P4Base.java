 ...
 * redirection functionality, and detect error based on return code functionality
 */
public abstract class P4Base extends org.apache.tools.ant.Task {
	...
	protected String descriptiveP4Opts = "";
	...
	/** If set this value, the return code for p4 commands are used to detect error. */
	private boolean detectErrorByRetCode = false;

	/**
	 * When redirects to log, use append mode or not
	 * default value is true
	 */
	protected boolean appendLog = true;
	...
	public boolean isDetectErrorByRetCode() {
		return detectErrorByRetCode;
	}

	/**
	 * Set if the return code of p4 command should be evaluated to detect error. If this value
	 * has been set to true, {@link this#setInError(boolean)} will be called to set an error
	 * when return value is other than 0
	 * @param detectErrorByRetCode
	 */
	public void setDetectErrorByRetCode(boolean detectErrorByRetCode) {
		this.detectErrorByRetCode = detectErrorByRetCode;
	}
	...
	public void setDescriptiveGlobalopts(String descriptiveP4Opts) {
		this.descriptiveP4Opts = descriptiveP4Opts;
	}
    ...
    */
    ...
    protected void execP4Command(String command, P4Handler handler) throws BuildException {
		try{
			...
			if (getP4Dir() == null || getP4Dir().trim().length() == 0)
            	commandline.setExecutable("p4");
			else
				commandline.setExecutable(getP4Dir().trim() + File.separator + "p4");
            ...
            if (P4Opts != null && P4Opts.length() != 0) {
				Commandline.Argument arg = commandline.createArgument();
                arg.setLine(P4Opts);
				if (descriptiveP4Opts != null && descriptiveP4Opts.length() != 0)
					arg.setDescriptiveLine(descriptiveP4Opts);
            }
            ...
            try {
				...
				if (retcode != 0 && !inError && detectErrorByRetCode){
					inError = true;
					errorMessage = "Execution of command failed, returned code: " + retcode;
				}
